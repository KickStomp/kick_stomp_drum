/*  Example for a bass kickdrum, adding potentiometers for parameter editing, switch wavetables
...
void setup(){
  ...
  SetKick(10,100,255, 600, 50, 25, 0); // 10ms attack, 100ms decay, full level, 600Hz, 50Hz, 25ms, sine wave
}
...
*/
void SetKick(uint16_t time1, uint16_t time2, uint8_t level, uint16_t f1, uint16_t f2, uint16_t time_gliss, uint8_t wave_table){
...
void updateControl(){
  ...
  static uint16_t time1 = 10, time2 = 100, level = 255, f1 = 600, f2 = 50, time_gliss = 25, wave_table = 0;
  static uint16_t bpm_speed = 80;
  
  static uint16_t note_length_4t = 750;
  ...
  uint8_t para_change = 0;
    ...
    if(switch1_mode){
      ...
      // Wave table map to 0..7, (AnalogRead : values 0..1024, divide by 2^7=128)
      analog_input = (mozziAnalogRead(analog_input_port[0]) >> 7); 
      ...
      if( analog_input != wave_table){
        ...
        if(delta_value(analog_input,wave_table) < 2){
          
          wave_table = analog_input;
          para_change = 1;

          Serial.print("Wavetable = "); Serial.println(wave_table);
        }
      }
     ...
     // Level map to 0..255, (AnalogRead : values 0..1024, divide by 2^2=4)
      ...
      // BPM-rate, (AnalogRead : values 0..255)
      analog_input = (mozziAnalogRead(analog_input_port[0]) >> 2); 
      if( analog_input != bpm_speed){
        if(delta_value(analog_input,bpm_speed) < 4){

          bpm_speed = analog_input;
          note_length_4t = (uint16_t)(60000) / bpm_speed;  // ( 4* 60 * 1000 / 4 ) / bpm
  
          Serial.print("BPM = "); Serial.println(bpm_speed);
        }
      }
    ...
    if(para_change){
      SetKick(time1, time2, uint8_t(level), f1, f2, time_gliss, wave_table); 
    }
...
int updateAudio(){
  ...
  new_audio = (int16_t) (
    ((
       ( (int16_t)(uint8_t)kick_wave_envelope.next() * (int16_t)(int8_t)kickOscil.next()  ) 
                                                             // 255 * 128 = 32640 == > / 4
